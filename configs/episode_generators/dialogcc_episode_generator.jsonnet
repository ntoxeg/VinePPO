local math_task = (import '../tasks/dialogcc.jsonnet') + {
    prepend_in_context_few_shot: false,
    ensure_fit_in_context_size: false,
};

local prompt_library = (import '../prompt_library/dialogcc_step_by_step_sft.jsonnet');
local question_template = prompt_library.prompt_library.tree.question_template;

{
    episode_generator+: {
        type: 'dialogcc_episode_generator',
        vllm_server+: {
            swap_space: 16,
        },
